      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features?.map((feature, index) => {
            const Icon =
              feature.icon && iconMap[feature.icon as keyof typeof iconMap]
        ...
        <div className="mt-16 bg-gradient-to-r from-primary/10 via-primary/5 to-primary/10 border border-primary/30 rounded-lg p-8 text-center">
          <h3 className="font-display text-2xl font-bold text-foreground mb-2">{cfpTitle}</h3>
          <p className="text-muted-foreground mb-4">{cfpDescription}</p>
          <CMSLink
            {...(cfpLinkProps as any)}
            appearance="inline"
